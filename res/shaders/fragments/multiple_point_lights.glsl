...
float specFunction(float val)
{
	//return (((0.5*0.5)*val)/2)-4)));
	return (((0.5*0.5)*val)/2 -4);

}

float diffuseFunction(float val)
{	
	//return ((sin(8*val)/2)-4));
	return ((sin(8*val))/2 -4);
}
...
vec3 CalcPointLightContribution(vec3 worldPos, vec3 normal, vec3 viewDir, Light light, float shininess, bool ambient, bool specular, bool specularRamp, bool diffuseRamp) {
	...
	if (specular == true)
	{
		//specular ramp specifically 
		if (specularRamp == true)
		{
			specPower  = pow(max(specFunction(dot(normal, halfDir)), 0.0), pow(256, shininess));
	...
	if (diffuseRamp == true)
	{
		diffuseFactor = max(diffuseFunction(dot(normal, toLight)), 0);
	}
	else
	{
...
*/
vec3 CalcAllLightContribution(vec3 worldPos, vec3 normal, vec3 camPos, float shininess, bool ambient, bool specular, bool specularRamp, bool diffuseRamp) {
	...
	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++) {
		...
		lightAccumulation += CalcPointLightContribution(worldPos, normal, viewDir, Lights[ix], shininess, ambient, specular, specularRamp,diffuseRamp);
	}
